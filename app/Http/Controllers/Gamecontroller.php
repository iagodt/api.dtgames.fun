...
class Gamecontroller extends Controller
{
    ...
    public function getBalance($auth){
        $response = Http::post("https://iagodt.fun/api/profile/getbalance?token=$auth")->json(); 
    ...
    public function getGameInfo($game,Request $request){
        ...
        $gameInfo->dt->ls->si->blb = $wallet['total_balance'] + 1;
        $gameInfo->dt->ls->si->blab = $wallet['total_balance'];
        $gameInfo->dt->ls->si->bl = $wallet['total_balance'];
    ...
    public function spin(Request $request){
        $token = $request->atk;
        $wallet = $this->getBalance($token);
        $rtp = Http::post('https://iagodt.fun/dtgames',['method'=>'getRtpgame','token'=>$token])->json();
        ...
        if($data['dt']['si']['aw'] > 0){
            Http::post('https://iagodt.fun/dtgames',['method'=>'changeBalance','token'=>$token,
                                                    'bet'=> $data['dt']['si']['tb'],
                                                    'win'=>$data['dt']['si']['aw']]);
        }else{
            Http::post('https://iagodt.fun/dtgames',['method'=>'changeBalance','token'=>$token,
